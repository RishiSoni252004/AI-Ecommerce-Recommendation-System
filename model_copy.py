...
class HybridRecommender:
    ...
    def update_user_recommendations(self, user_id):
        ...
        # Get items the user has already interacted with — we EXCLUDE these
        interacted_items = set()
        if not self.interactions_df.empty:
            user_interactions = self.interactions_df[self.interactions_df["user_id"] == user_id]
            interacted_items = set(user_interactions["item_id"].unique())
        
        top_items = []
        # If user has zero interactions, they should "start fresh" with no recommendations
        if np.max(scores) > 0:
            ...
            for idx in item_indices:
                ...
                # SKIP items the user already viewed/clicked/purchased
                if item_id in interacted_items:
                    continue
                
                item_data = self.items_df.iloc[idx].to_dict()
                item_data = {k: float(v) if isinstance(v, (np.floating, float)) else int(v) if isinstance(v, (np.integer, int)) else v for k, v in item_data.items()}
                item_data["item_id"] = item_id
                top_items.append(item_data)
                count += 1
            
        # Store in Redis for instant load
        redis_client.set_recommendations(user_id, top_items)
        # Store in MongoDB for permanent persistence
        db_client.update_user_recommendations(user_id, top_items)
